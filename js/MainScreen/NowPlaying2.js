...
export default class NowPlaying {
    constructor(store, trackList, audio) {
        this.store = store;
        this.audio = audio;
        this.songs = trackList;

        this.song = new Song(this.store, this.songs[0], this.audio);
    ...
    getCurrIndexSong() {
        return this.store.states.playingSong;
    }

    setCurrIndexSong(index) {
        this.store.dispatch("SET_PLAYING_SONG", index);
    }
